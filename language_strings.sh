...
function language_strings() {
	...
	arr["CATALAN",610]="També es pot utilitzar john the ripper per desxifrar els hashes obtinguts en un atac a una xarxa wifi enterprise. Atacs de diccionari, basats en regles o per força bruta poden ser usats des dels menús de airgeddon"
	arr["PORTUGUESE",610]="Você também pode usar o john the ripper para descriptografar as hashes obtidas em um ataque a uma rede wifi enterprise. Ataques de dicionário, baseados em regras ou força bruta, podem ser usados nos menus do airgeddon"
