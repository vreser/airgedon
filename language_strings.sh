...
function language_strings() {
	...
	arr["FRENCH",549]="${pending_of_translation} 6.  (asleap) Attaque du dictionnaire défi/réponse"
	...
	arr["FRENCH",610]="${pending_of_translation} Vous pouvez également utiliser john the ripper pour déchiffrer les hachages obtenus lors d'une attaque contre un réseau wifi d'entreprise. Les attaques de dictionnaire ou la force brute peuvent être utilisées à partir des menus de airgeddon"
	...
	arr["RUSSIAN",610]="${pending_of_translation} Хэши, полученные во время атаки на корпоративную сеть wifi, также могут быть расшифрованы с помощью инструмента john the ripper. Атаки по словарю или грубой силой можно использовать из меню аэрогеддона"
	...
	arr["ITALIAN",610]="${pending_of_translation} È inoltre possibile utilizzare john the ripper per decrittografare gli hash ottenuti in un attacco a una rete wifi aziendale. Gli attacchi di dizionario o forza bruta, possono essere utilizzati dai menu di airgeddon"
	...
	arr["GERMAN",610]="${pending_of_translation} Sie können john the ripper auch verwenden, um die bei einem Angriff auf ein Wi-Fi-Netzwerk eines Unternehmens erhaltenen Hashes zu entschlüsseln. Wörterbuchangriffe oder Brute Force basieren, können aus den Menüs von airgeddon verwendet werden"
